...
public sealed class MainForm : Form
{
    ...
    private void ChooseProject()
    {
        using var dialog = new FolderBrowserDialog {
            Description = "Selecciona la carpeta raíz de MODGD"
        };
    ...
    private async Task BuildAsync()
    {
        ...
        {
            MessageBox.Show(
                "No encontré mod.json y CMakeLists.txt en la carpeta seleccionada.",
                "MODGD Builder",
        ...
        try
        {
            ...
            {
                Log("Geode CLI no está instalado.");
                Log("Instálalo con: winget install GeodeSDK.GeodeCLI");
                MessageBox.Show(
                    "Falta Geode CLI. Instálalo con Winget y vuelve a abrir el Builder.",
                    "Geode CLI requerido",
                    MessageBoxButtons.OK,
            ...
            Log("Instalando/actualizando SDK...");
            if (!await RunAsync("geode", "sdk install"))
                throw new Exception("No se pudo instalar el Geode SDK.");

            Log("Comprobando dependencias...");
            if (!await RunAsync("geode", "project check -p windows"))
                Log("Aviso: la comprobación de dependencias terminó con errores.");

            Log("Compilando para Windows x64...");
            if (!await RunAsync("geode", "build -p windows --config Release"))
                throw new Exception("La compilación falló.");

            var geode = Directory
                .EnumerateFiles(Path.Combine(projectDir, "build"), "*.geode", SearchOption.AllDirectories)
                .OrderByDescending(File.GetLastWriteTimeUtc)
                .FirstOrDefault();

            if (geode is null)
                throw new Exception("La compilación terminó pero no apareció ningún archivo .geode.");
